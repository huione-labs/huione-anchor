...
use huione_anchor_client::Cluster;
use huione_anchor_lang::idl::{IdlAccount, IdlInstruction, ERASED_AUTHORITY};
use huione_anchor_lang::{AccountDeserialize, AnchorDeserialize, AnchorSerialize};
use huione_anchor_syn::idl::Idl;
...
use huione_client::rpc_client::RpcClient;
use huione_client::rpc_config::RpcSendTransactionConfig;
use huione_program::instruction::{AccountMeta, Instruction};
use huione_sdk::account_utils::StateMut;
use huione_sdk::bpf_loader;
use huione_sdk::bpf_loader_deprecated;
use huione_sdk::bpf_loader_upgradeable::{self, UpgradeableLoaderState};
use huione_sdk::commitment_config::CommitmentConfig;
use huione_sdk::pubkey::Pubkey;
use huione_sdk::signature::Keypair;
use huione_sdk::signature::Signer;
use huione_sdk::sysvar;
use huione_sdk::transaction::Transaction;
...
pub enum Command {
    ...
    Build {
        ...
        /// Version of the HUIONE toolchain to use. For --verifiable builds
        /// only.
        #[clap(short = 'c', long)]
        huione_version: Option<String>,
...
pub fn build(
    ...
    let build_config = BuildConfig {
        verifiable,
        huione_version: huione_version.or_else(|| cfg.huione_version.clone()),
...
fn docker_prep(container_name: &str, build_config: &BuildConfig) -> Result<()> {
    // Set the huione version in the container, if given. Otherwise use the
    // default.
    ...
    if let Some(huione_version) = &build_config.huione_version {
        println!("Using huione version: {}", huione_version);
        ...
        docker_exec(
            ...
            &[
                "curl",
                "-sSfL",
                &format!("https://release.huione.com/v{0}/install", huione_version,),
                "-o",
                "huione_installer.sh",
            ],
        )?;
        docker_exec(container_name, &["sh", "huione_installer.sh"])?;
        docker_exec(container_name, &["rm", "-f", "huione_installer.sh"])?;
    }
    Ok(())
}
...
fn docker_build_bpf(
    ...
    let exit = std::process::Command::new("docker")
        .args(&[
            "exec",
            "--env",
            "PATH=/root/.local/share/huione/install/active_release/bin:/root/.cargo/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
            container_name,
            "cargo",
            "build-bpf",
            "--manifest-path",
...
fn verify(
    ...
    build(
        ...
        true,                                                  // skip lint
        None,                                                  // program name
        huione_version.or_else(|| cfg.huione_version.clone()), // huione version
...
fn fetch_idl(cfg_override: &ConfigOverride, idl_addr: Pubkey) -> Result<Idl> {
    let url = match Config::discover(cfg_override)? {
        ...
        None => {
            ...
            // cluster_url will be used from default huione config
            // provider.cluster option can be used to override this

            if let Some(cluster) = cfg_override.cluster.clone() {
                cluster.url().to_string()
            } else {
                config::get_huione_cfg_url()?
            }
        }
    };
...
fn extract_idl(cfg: &WithPath<Config>, file: &str, skip_lint: bool) -> Result<Option<Idl>> {
    ...
    huione_anchor_syn::idl::file::parse(&*file, cargo.version(), cfg.features.seeds, !skip_lint)
}
...
fn idl_set_buffer(cfg_override: &ConfigOverride, program_id: Pubkey, buffer: Pubkey) -> Result<()> {
    with_workspace(cfg_override, |cfg| {
        let keypair = huione_sdk::signature::read_keypair_file(&cfg.provider.wallet.to_string())
        ...
        let set_buffer_ix = {
            ...
            let mut data = huione_anchor_lang::idl::IDL_IX_TAG.to_le_bytes().to_vec();
...
fn idl_set_authority(
    ...
    with_workspace(cfg_override, |cfg| {
        ...
        // Instruction data.
        let data =
            serialize_idl_ix(huione_anchor_lang::idl::IdlInstruction::SetAuthority { new_authority })?;
...
fn idl_write(cfg: &Config, program_id: &Pubkey, idl: &Idl, idl_address: Pubkey) -> Result<()> {
    ...
    while offset < idl_data.len() {
        ...
        let data = {
            ...
            serialize_idl_ix(huione_anchor_lang::idl::IdlInstruction::Write {
...
// Returns the huione-test-validator flags. This will embed the workspace
// programs in the genesis block so we don't have to deploy every time. It also
// allows control of other huione-test-validator features.
fn validator_flags(
    cfg: &WithPath<Config>,
    test_validator: &Option<TestValidator>,
) -> Result<Vec<String>> {
            ...
            for (key, value) in entries.as_object().unwrap() {
                ...
                if key == "account" {
                    ...
                    let client = if let Some(url) = entries["url"].as_str() {
                        ...
                        return Err(anyhow!(
                    "Validator url for HUIONE's JSON RPC should be provided in order to clone accounts from it"
                ));
                    };
...
fn stream_logs(config: &WithPath<Config>, rpc_url: &str) -> Result<Vec<std::process::Child>> {
    ...
    for program in config.read_all_programs()? {
        ...
        let child = std::process::Command::new("huione")
            .arg("logs")
...
fn start_test_validator(
    ...
    let rpc_port = cfg
        ...
        .unwrap_or(huione_sdk::rpc_port::DEFAULT_RPC_PORT);
    ...
    let faucet_port = cfg
        ...
        .unwrap_or(huione_faucet::faucet::FAUCET_PORT);
    ...
    let mut validator_handle = std::process::Command::new("huione-test-validator")
        .arg("--ledger")
...
// Return the URL that huione-test-validator should be running on given the
// configuration
...
// Setup and return paths to the huione-test-validator ledger directory and log
...
fn deploy(cfg_override: &ConfigOverride, program_str: Option<String>) -> Result<()> {
    with_workspace(cfg_override, |cfg| {
        ...
        for mut program in cfg.read_all_programs()? {
            ...
            let exit = std::process::Command::new("huione")
                .arg("program")
                .arg("deploy")
                .arg("--url")
                .arg(&url)
...
fn create_idl_account(
    ...
    let keypair = huione_sdk::signature::read_keypair_file(keypair_path)
        .map_err(|_| anyhow!("Unable to read keypair file"))?;
    let url = cluster_url(cfg, &cfg.test_validator);
    let client = RpcClient::new(url);
    ...
    {
        let data = serialize_idl_ix(huione_anchor_lang::idl::IdlInstruction::Create {
        ...
        let accounts = vec![
            ...
            AccountMeta::new_readonly(huione_program::sysvar::rent::ID, false),
        ];
        let ix = Instruction {
            program_id: *program_id,
            accounts,
...
fn create_idl_buffer(
    ...
    let create_account_ix = {
        ...
        huione_sdk::system_instruction::create_account(
            &keypair.pubkey(),
            &buffer.pubkey(),
            lamports,
            space as u64,
...
fn serialize_idl_ix(ix_inner: huione_anchor_lang::idl::IdlInstruction) -> Result<Vec<u8>> {
    let mut data = huione_anchor_lang::idl::IDL_IX_TAG.to_le_bytes().to_vec();
