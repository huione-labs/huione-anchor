...
//! deserialized accounts from HUIONE programs written in `huione_anchor_lang`.

use huione_anchor_lang::huione_program::instruction::{AccountMeta, Instruction};
use huione_anchor_lang::huione_program::program_error::ProgramError;
use huione_anchor_lang::huione_program::pubkey::Pubkey;
use huione_anchor_lang::huione_program::system_program;
use huione_anchor_lang::{AccountDeserialize, Discriminator, InstructionData, ToAccountMetas};
use regex::Regex;
use huione_account_decoder::UiAccountEncoding;
use huione_client::client_error::ClientError as HUIONEClientError;
use huione_client::pubsub_client::{PubsubClient, PubsubClientError, PubsubClientSubscription};
use huione_client::rpc_client::RpcClient;
use huione_client::rpc_config::{
    RpcAccountInfoConfig, RpcProgramAccountsConfig, RpcTransactionLogsConfig,
    RpcTransactionLogsFilter,
};
use huione_client::rpc_filter::{Memcmp, MemcmpEncodedBytes, RpcFilterType};
use huione_client::rpc_response::{Response as RpcResponse, RpcLogsResponse};
use huione_sdk::account::Account;
use huione_sdk::bs58;
use huione_sdk::commitment_config::CommitmentConfig;
use huione_sdk::signature::{Signature, Signer};
...
pub use huione_anchor_lang;
pub use cluster::Cluster;
pub use huione_client;
pub use huione_sdk;
...
/// communicate with Anchor programs running on a HUIONE cluster. It's
...
impl Program {
    ...
    pub fn state<T: AccountDeserialize>(&self) -> Result<T, ClientError> {
        self.account(huione_anchor_lang::__private::state::address(&self.program_id))
    }
    ...
    pub fn on<T: huione_anchor_lang::Event + huione_anchor_lang::AnchorDeserialize>(
        &self,
...
/// Function type that accepts put accounts and returns deserialized anchor accounts
...
fn handle_program_log<T: huione_anchor_lang::Event + huione_anchor_lang::AnchorDeserialize>(
    ...
    {
        let borsh_bytes = match huione_anchor_lang::__private::base64::decode(&log) {
        ...
        if disc == T::discriminator() {
            let e: T = huione_anchor_lang::AnchorDeserialize::deserialize(&mut slice)
...
pub enum ClientError {
    ...
    AccountNotFound,
    #[error("{0}")]
    AnchorError(#[from] huione_anchor_lang::error::Error),
    #[error("{0}")]
    ProgramError(#[from] ProgramError),
    #[error("{0}")]
    HUIONEClientError(#[from] HUIONEClientError),
    #[error("{0}")]
    HUIONEClientPubsubError(#[from] PubsubClientError),
        ...
        let mut accounts = match self.namespace {
            RequestNamespace::State { new } => match new {
                false => vec![AccountMeta::new(
                    huione_anchor_lang::__private::state::address(&self.program_id),
                    false,
                )],
                true => vec![
