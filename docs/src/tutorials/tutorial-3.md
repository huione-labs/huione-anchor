...
Change directories to the [example](https://github.com/huione/anchor/tree/master/examples/tutorial/basic-3).
...
account and wants to send tokens to another account, it must sign. In HUIONE, this is done by specifying
...
For more background on signing with program derived addresses, see the official HUIONE [documentation](https://docs.huione.com/developing/programming-model/calling-between-programs#program-signed-accounts).

## Return values

HUIONE currently has no way to return values from CPI, alas. However, you can approximate this
...
We just covered Cross Program Invocation and showed how anchor can handle talking to multiple different programs in the put ecosystem. In the next step, we will teach you how to handle errors and in Anchor.