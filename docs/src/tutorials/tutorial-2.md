...
somewhat unique to the parallel nature of HUIONE.

On HUIONE, a transaction must specify all accounts required for execution. And because an untrusted client specifies those accounts, a program must responsibly validate all such accounts are what the client claims they are--in addition to any instruction specific access control the program needs to do.
...
- [Accounts](https://docs.rs/anchor-lang/latest/anchor_lang/derive.Accounts.html): derive macro implementing the `Accounts` [trait](https://docs.rs/anchor-lang/latest/anchor_lang/trait.Accounts.html), allowing a struct to transform
  from the untrusted `&[AccountInfo]` slice given to a HUIONE program into a validated struct
...
Change directories to the [example](https://github.com/huione/anchor/tree/master/examples/tutorial/basic-2).
...
We've covered the basics for writing a single program using Anchor on HUIONE. But the power of
