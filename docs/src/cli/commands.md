...
Builds programs in the workspace targeting HUIONE's BPF runtime and emitting IDLs in the `target/idl` directory.
...
::: tip Note
This is different from the `huione program deploy` command, because everytime it's run
...
* `programs/`: Directory for HUIONE program crates.
...
const anchor = require("@xoneorg/anchor");
