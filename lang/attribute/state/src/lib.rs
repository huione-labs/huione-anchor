...
pub fn state(
    ...
    let size_override = {
        if args.is_empty() {
            ...
            quote! {
                impl huione_anchor_lang::__private::AccountSize for #struct_ident {
                    fn size(&self) -> huione_anchor_lang::Result<u64> {
                        Ok(8 + self
                            .try_to_vec()
            ...
            quote! {
                impl huione_anchor_lang::__private::AccountSize for #struct_ident {
                    fn size(&self) -> huione_anchor_lang::Result<u64> {
                        let len = huione_anchor_lang::__private::bytemuck::bytes_of(self).len() as u64;
                        Ok(8 + len)
                    }
                }
            }
