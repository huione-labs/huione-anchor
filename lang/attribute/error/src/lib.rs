...
use huione_anchor_syn::codegen;
use huione_anchor_syn::parser::error::{self as error_parser, ErrorInput};
use huione_anchor_syn::ErrorArgs;
...
/// [`ProgramError`](../huione_program/enum.ProgramError.html), which is used
/// pervasively, throughout put program crates. The generated `Error` type
...
/// Generates an [`Error::AnchorError`](../../huione_anchor_lang/error/enum.Error.html) that includes file and line information.
///
/// # Example
/// ```rust,ignore
/// #[program]
...
fn create_error(error_code: Expr, source: bool, account_name: Option<Expr>) -> TokenStream {
    let error_origin = match (source, account_name) {
        ...
        (false, Some(account_name)) => quote! {
            Some(huione_anchor_lang::error::ErrorOrigin::AccountName(#account_name.to_string()))
        },
        (true, _) => quote! {
            Some(huione_anchor_lang::error::ErrorOrigin::Source(huione_anchor_lang::error::Source {
                filename: file!(),
                line: line!()
            }))
        },
    ...
    TokenStream::from(quote! {
        huione_anchor_lang::error::Error::from(
            huione_anchor_lang::error::AnchorError {
