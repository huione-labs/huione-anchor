//! Copied from huione/sdk/macro so that Anchor programs don't need to specify
//! `huione_program` as an additional crate dependency, but instead can access
//! it via `huione_anchor_lang::declare_id`.
//!
...
impl Parse for Id {
    fn parse(input: ParseStream) -> Result<Self> {
        parse_id(
            input,
            quote! { huione_anchor_lang::huione_program::pubkey::Pubkey },
        )
        .map(Self)
    }
}
...
impl Parse for Pubkeys {
    fn parse(input: ParseStream) -> Result<Self> {
        let pubkey_type = quote! {
            huione_anchor_lang::huione_program::pubkey::Pubkey
        };
