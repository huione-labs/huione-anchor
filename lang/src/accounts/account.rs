...
use huione_program::system_program;
use std::collections::BTreeMap;
use std::fmt;
use std::ops::{Deref, DerefMut};

/// Wrapper around [`AccountInfo`](crate::huione_program::account_info::AccountInfo)
...
/// to write.
///
/// The mint wrapper type that Anchor provides out of the box for the token program ([source](https://github.com/huione/anchor/blob/master/spl/src/token.rs))
/// ```ignore
/// #[derive(Clone)]
...
impl<'a, T: AccountSerialize + AccountDeserialize + Owner + Clone> DerefMut for Account<'a, T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        ...
        if !self.info.is_writable {
            huione_program::msg!("The given Account is not mutable");
            panic!();
        }
        &mut self.account
    }
