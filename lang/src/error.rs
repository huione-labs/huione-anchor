use huione_anchor_lang::error_code;
use borsh::maybestd::io::Error as BorshIoError;
use huione_program::{program_error::ProgramError, pubkey::Pubkey};
...
impl ProgramErrorWithOrigin {
    pub fn log(&self) {
        match &self.error_origin {
            None => {
                huione_anchor_lang::huione_program::msg!(
            ...
            Some(ErrorOrigin::AccountName(account_name)) => {
                // using huione_log because msg! wrongly interprets 5 inputs as u64
                huione_anchor_lang::huione_program::log::huione_log(&format!(
        ...
        match &self.compared_values {
            Some(ComparedValues::Pubkeys((left, right))) => {
                huione_anchor_lang::huione_program::msg!("Left:");
                left.log();
                huione_anchor_lang::huione_program::msg!("Right:");
                right.log();
            }
            Some(ComparedValues::Values((left, right))) => {
                huione_anchor_lang::huione_program::msg!("Left: {}", left);
                huione_anchor_lang::huione_program::msg!("Right: {}", right);
            }
            None => (),
        }
    }
...
impl std::convert::From<Error> for huione_anchor_lang::huione_program::program_error::ProgramError {
    fn from(e: Error) -> huione_anchor_lang::huione_program::program_error::ProgramError {
        match e {
            Error::AnchorError(AnchorError {
                error_code_number, ..
            }) => {
                huione_anchor_lang::huione_program::program_error::ProgramError::Custom(error_code_number)
            }
