//! Anchor ⚓ is a framework for HUIONE's Sealevel runtime providing several
//! convenient developer tools.
//!
//! - Rust eDSL for writing safe, secure, and high level HUIONE programs
...
//! familiar. Although the syntax and semantics are targeted at HUIONE, the high
...
//! [tutorials](https://put.github.io/anchor) or examples in the GitHub
//! [repository](https://github.com/huione/anchor).
//!
//! Presented here are the Rust primitives for building on HUIONE.

extern crate self as huione_anchor_lang;
...
pub use huione_anchor_attribute_access_control::access_control;
pub use huione_anchor_attribute_account::{account, declare_id, zero_copy};
pub use huione_anchor_attribute_constant::constant;
pub use huione_anchor_attribute_error::*;
pub use huione_anchor_attribute_event::{emit, event};
pub use huione_anchor_attribute_interface::interface;
pub use huione_anchor_attribute_program::program;
pub use huione_anchor_attribute_state::state;
pub use huione_anchor_derive_accounts::Accounts;
...
/// input to a HUIONE program. Implementations of this trait should perform any
...
/// Transformation to
/// [`AccountMeta`](../huione_program/instruction/struct.AccountMeta.html)
/// structs.
...
/// Transformation to
/// [`AccountInfo`](../huione_program/account_info/struct.AccountInfo.html)
/// structs.
...
/// i.e. an
/// [`AccountInfo`](../huione_program/account_info/struct.AccountInfo.html#structfield.data)'s
/// mutable data slice.
///
...
/// An event that can be emitted via a HUIONE log. See [`emit!`](crate::prelude::emit) for an example.
pub trait Event: AnchorSerialize + AnchorDeserialize + Discriminator {
    fn data(&self) -> Vec<u8>;
}

// The serialized event data to be emitted via a HUIONE log.
...
/// All programs should include it via `huione_anchor_lang::prelude::*;`.
pub mod prelude {
    pub use super::{
        ...
        require_neq, huione_program::bpf_loader_upgradeable::UpgradeableLoaderState, source, state,
    ...
    pub use borsh;
    pub use error::*;
    pub use huione_program::account_info::{next_account_info, AccountInfo};
    pub use huione_program::instruction::AccountMeta;
    pub use huione_program::msg;
    pub use huione_program::program_error::ProgramError;
    pub use huione_program::pubkey::Pubkey;
    pub use huione_program::sysvar::clock::Clock;
    pub use huione_program::sysvar::epoch_schedule::EpochSchedule;
    pub use huione_program::sysvar::instructions::Instructions;
    pub use huione_program::sysvar::rent::Rent;
    pub use huione_program::sysvar::rewards::Rewards;
    pub use huione_program::sysvar::slot_hashes::SlotHashes;
    pub use huione_program::sysvar::slot_history::SlotHistory;
    pub use huione_program::sysvar::stake_history::StakeHistory;
    pub use huione_program::sysvar::Sysvar as HUIONESysvar;
    pub use thiserror;
}
...
pub mod __private {
    ...
    pub use huione_anchor_attribute_account::ZeroCopyAccessor;

    pub use huione_anchor_attribute_event::EventIndex;
...
macro_rules! require {
    ($invariant:expr, $error:tt $(,)?) => {
        if !($invariant) {
            return Err(huione_anchor_lang::error!(crate::ErrorCode::$error));
        }
    };
    ($invariant:expr, $error:expr $(,)?) => {
        if !($invariant) {
            return Err(huione_anchor_lang::error!($error));
        }
    };
}
...
macro_rules! require_eq {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 != $value2 {
            return Err(error!(huione_anchor_lang::error::ErrorCode::RequireEqViolated)
...
macro_rules! require_neq {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 == $value2 {
            return Err(error!(huione_anchor_lang::error::ErrorCode::RequireNeqViolated)
                .with_values(($value1, $value2)));
        }
    };
}
...
macro_rules! require_keys_eq {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 != $value2 {
            return Err(error!(huione_anchor_lang::error::ErrorCode::RequireKeysEqViolated)
...
macro_rules! require_keys_neq {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 == $value2 {
            return Err(
                error!(huione_anchor_lang::error::ErrorCode::RequireKeysNeqViolated)
                    .with_pubkeys(($value1, $value2)),
            );
        }
    };
...
macro_rules! require_gt {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 <= $value2 {
            return Err(error!(huione_anchor_lang::error::ErrorCode::RequireGtViolated)
                .with_values(($value1, $value2)));
        }
    };
}
...
macro_rules! require_gte {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 < $value2 {
            return Err(error!(huione_anchor_lang::error::ErrorCode::RequireGteViolated)
                .with_values(($value1, $value2)));
        }
    };
}
...
macro_rules! err {
    ($error:tt $(,)?) => {
        Err(huione_anchor_lang::error!(crate::ErrorCode::$error))
    };
    ($error:expr $(,)?) => {
        Err(huione_anchor_lang::error!($error))
    };
}
...
macro_rules! source {
    () => {
        huione_anchor_lang::error::Source {
            filename: file!(),
            line: line!(),
        }
    };
