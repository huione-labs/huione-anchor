...
pub fn parse_token(stream: ParseStream) -> ParseResult<ConstraintToken> {
    ...
    let c = match kw.as_str() {
        ...
        "meta" => {
            ...
            match kw.as_str() {
                "authority" => ConstraintToken::MetaAuth(Context::new(
                    span,
                    ConstraintTokenAuthority {
                        auth: stream.parse()?,
                    },
                )),
                "mint" => ConstraintToken::MetaMint(Context::new(
                    span,
                    ConstraintMetaMint {
                        meta_mint: stream.parse()?,
                    },
                )),
                "symbol" => ConstraintToken::MetaSymbol(Context::new(
                    span,
                    ConstraintMetaSymbol {
                        meta_symbol: stream.parse()?,
                    },
                )),
                "name" => ConstraintToken::MetaName(Context::new(
                    span,
                    ConstraintMetaName {
                        meta_name: stream.parse()?,
                    },
                )),
                "icon" => ConstraintToken::MetaIcon(Context::new(
                    span,
                    ConstraintMetaIcon {
                        meta_icon: stream.parse()?,
                    },
                )),
                _ => return Err(ParseError::new(ident.span(), "Invalid attribute")),
            }
...
pub struct ConstraintGroupBuilder<'ty> {
    ...
    pub meta_authority:Option<Context<ConstraintTokenAuthority>>,
    pub meta_mint: Option<Context<ConstraintMetaMint>>,
    pub meta_symbol: Option<Context<ConstraintMetaSymbol>>,
    pub meta_name: Option<Context<ConstraintMetaName>>,
    pub meta_icon: Option<Context<ConstraintMetaIcon>>,
}
...
impl<'ty> ConstraintGroupBuilder<'ty> {
    ...
    pub fn build(mut self) -> ParseResult<ConstraintGroup> {
        ...
        if let Some(i) = &self.init {
            ...
            // account instruction.
            if self.signer.is_none() && self.seeds.is_none() && self.associated_token_mint.is_none() && self.meta_mint.is_none()
            {
                self.signer
            ...
            // if let Some(mint_symbol) = &self.mint_symbol {
            //     if self.mint_authority.is_none() {
            //         return Err(ParseError::new(
            //             mint_symbol.span(),
            //             "when initializing, mint authority must be provided if mint decimals is",
            ...
            //     }
            // }
            // if let Some(mint_name) = &self.mint_name {
            //     if self.mint_authority.is_none() {
            //         return Err(ParseError::new(
            //             mint_name.span(),
            //             "when initializing, mint authority must be provided if mint decimals is",
            ...
            //     }
            // }
            // if let Some(mint_icon) = &self.mint_icon {
            //     if self.mint_authority.is_none() {
            //         return Err(ParseError::new(
            //             mint_icon.span(),
            //             "when initializing, mint authority must be provided if mint decimals is",
        ...
        let mint_meta_group = match (meta_authority,meta_mint ,meta_symbol, meta_name,meta_icon) {
            (None, None, None, None, None) => None,
            ( Some(auth),Some(mint), Some(symbol), Some(name),Some(icon)) => Some(ConstraintTokenMintMetaGroup {
                meta_authority: auth.into_inner().auth,
                meta_mint: mint.into_inner().meta_mint,
                meta_symbol: symbol.into_inner().meta_symbol,
                meta_name: name.into_inner().meta_name,
                meta_icon: icon.into_inner().meta_icon,
            }),
            (_,Some(mint),_, _,_) => {
                return Err(ParseError::new(
                    mint.span(),
                    "auth, mint, symbol, name, icon, must be provided to specify for mint meta.",
                ))
            },
            (_,_,Some(symbol), _,_) => {
                return Err(ParseError::new(
        ...
        Ok(ConstraintGroup {
            init: init.as_ref().map(|i| Ok(ConstraintInitGroup {
                ...
                } else if let Some(mm) = &mint_meta_group {
                    InitKind::MintMeta{
                        authority:mm.meta_authority.clone(),
                        mint: mm.meta_mint.clone(),
                        symbol: mm.meta_symbol.clone(),
                        name: mm.meta_name.clone(),
                        icon: mm.meta_icon.clone(),
                    }
                } else {
                    InitKind::Program {
    ...
    pub fn add(&mut self, c: ConstraintToken) -> ParseResult<()> {
        match c {
            ...
            ConstraintToken::MetaAuth(c) => self.add_meta_authority(c),
            ConstraintToken::MetaMint(c) => self.add_meta_mint(c),
            ConstraintToken::MetaSymbol(c) => self.add_meta_symbol(c),
            ConstraintToken::MetaName(c) => self.add_meta_name(c),
            ConstraintToken::MetaIcon(c) => self.add_meta_icon(c),
        }
    }
    ...
    fn add_init(&mut self, c: Context<ConstraintInit>) -> ParseResult<()> {
        ...
        if self.meta_symbol.is_some() {
            return Err(ParseError::new(
                c.span(),
                "init must be provided before mint symbol",
            ));
        }
        if self.meta_name.is_some() {
            return Err(ParseError::new(
                c.span(),
                "init must be provided before mint name",
            ));
        }
        if self.meta_icon.is_some() {
            return Err(ParseError::new(
                c.span(),
                "init must be provided before mint icon",
            ));
        }
        self.init.replace(c);
        Ok(())
    ...
    fn add_meta_mint(&mut self, c: Context<ConstraintMetaMint>) -> ParseResult<()> {
        if self.meta_mint.is_some() {
            return Err(ParseError::new(c.span(), "mint symbol already provided"));
        }
        self.meta_mint.replace(c);
        Ok(())
    }


    fn add_meta_symbol(&mut self, c: Context<ConstraintMetaSymbol>) -> ParseResult<()> {
        if self.meta_symbol.is_some() {
            return Err(ParseError::new(c.span(), "mint symbol already provided"));
        }
        self.meta_symbol.replace(c);
        Ok(())
    }

    fn add_meta_name(&mut self, c: Context<ConstraintMetaName>) -> ParseResult<()> {
        if self.meta_name.is_some() {
            return Err(ParseError::new(c.span(), "mint name already provided"));
        }
        self.meta_name.replace(c);
        Ok(())
    }

    fn add_meta_icon(&mut self, c: Context<ConstraintMetaIcon>) -> ParseResult<()> {
        if self.meta_icon.is_some() {
            return Err(ParseError::new(c.span(), "mint icon already provided"));
        }
        self.meta_icon.replace(c);
        Ok(())
    }

    fn add_meta_authority(&mut self, c: Context<ConstraintTokenAuthority>) -> ParseResult<()> {
        if self.meta_authority.is_some() {
            return Err(ParseError::new(c.span(), "mint icon already provided"));
        }
        self.meta_authority.replace(c);
        Ok(())
    }
