                        ...
                        .map(|method: &StateIx| {
                            ...
                            quote! {
                                pub fn #method_name<'a, 'b, 'c, 'info>(
                                    ctx: huione_anchor_lang::context::CpiStateContext<'a, 'b, 'c, 'info, #accounts_ident<'info>>,
                                    #(#args),*
                                ) -> huione_anchor_lang::Result<()> {
                                    let ix = {
                                        let ix = instruction::state::#ix_variant;
                                        let data = huione_anchor_lang::InstructionData::data(&ix);
                                        let accounts = ctx.to_account_metas(None);
                                        huione_anchor_lang::huione_program::instruction::Instruction {
                                            program_id: crate::ID,
                                            accounts,
                                            data,
                                        }
        ...
        .map(|ix| {
            ...
            let cpi_method = {
                ...
                let (method_ret, maybe_return) = match ret_type.to_string().as_str() {
                    "()" => (quote! {huione_anchor_lang::Result<()> }, quote! { Ok(()) }),
                    _ => (
                        quote! { huione_anchor_lang::Result<crate::cpi::Return::<#ret_type>> },
                ...
                quote! {
                    pub fn #method_name<'a, 'b, 'c, 'info>(
                        ctx: huione_anchor_lang::context::CpiContext<'a, 'b, 'c, 'info, #accounts_ident<'info>>,
                        #(#args),*
                    ) -> #method_ret {
                        let ix = {
                            let ix = instruction::#ix_variant;
                            let mut ix_data = AnchorSerialize::try_to_vec(&ix)
                                .map_err(|_| huione_anchor_lang::error::ErrorCode::InstructionDidNotSerialize)?;
                        ...
                        huione_anchor_lang::huione_program::program::invoke_signed(
                            ...
                            |e| Err(Into::into(e)),
                            // Maybe handle HUIONE return data.
                            |_| { #maybe_return }
                        )
                    }
                }
    ...
    quote! {
        ...
        pub mod cpi {
            ...
            impl<T: AnchorDeserialize> Return<T> {
                pub fn get(&self) -> T {
                    let (_key, data) = huione_anchor_lang::huione_program::program::get_return_data().unwrap();
