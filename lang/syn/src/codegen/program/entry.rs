...
pub fn generate(program: &Program) -> proc_macro2::TokenStream {
    ...
    let fallback_maybe = dispatch::gen_fallback(program).unwrap_or(quote! {
        Err(huione_anchor_lang::error::ErrorCode::InstructionMissing.into())
    });
    quote! {
        #[cfg(not(feature = "no-entrypoint"))]
        huione_anchor_lang::huione_program::entrypoint!(entry);
        ...
        ///   is a variant of sighash. See docs.rs for `huione_anchor_lang` for details.
        ...
        ///
        /// The `entry` function here, defines the standard entry to a HUIONE
        /// program, where execution begins.
        pub fn entry(program_id: &Pubkey, accounts: &[AccountInfo], data: &[u8]) -> huione_anchor_lang::huione_program::entrypoint::ProgramResult {
        ...
        fn try_entry(program_id: &Pubkey, accounts: &[AccountInfo], data: &[u8]) -> huione_anchor_lang::Result<()> {
            #[cfg(feature = "anchor-debug")]
            {
                msg!("anchor-debug is active");
            }
            if *program_id != ID {
                return Err(huione_anchor_lang::error::ErrorCode::DeclaredProgramIdMismatch.into());
            }
            if data.len() < 8 {
        ...
        pub mod program {
            ...
            impl huione_anchor_lang::Id for #name {
                fn id() -> Pubkey {
                    ID
                }
            }
