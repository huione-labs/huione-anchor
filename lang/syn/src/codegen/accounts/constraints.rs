...
pub fn linearize(c_group: &ConstraintGroup) -> Vec<Constraint> {
    ...
    if let Some(c) = mint_meta {
        constraints.push(Constraint::MintMetaGroupConstraint(c));
    }
    constraints
}
...
fn generate_constraint(f: &Field, c: &Constraint) -> proc_macro2::TokenStream {
    match c {
        ...
        Constraint::MintMetaGroupConstraint(c) => generate_constraint_mint_meta(f, c),
    }
}
    ...
    quote! {
        let #field: #ty_decl = {
            ...
            if __discriminator != 0 {
                return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintZero).with_account_name(#name_str));
            }
            #from_account_info
        };
    }
...
pub fn generate_constraint_close(f: &Field, c: &ConstraintClose) -> proc_macro2::TokenStream {
    ...
    quote! {
        if #field.key() == #target.key() {
            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintClose).with_account_name(#name_str));
        }
    }
}
...
pub fn generate_constraint_literal(
    ...
    quote! {
        if !(#lit) {
            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::Deprecated).with_account_name(#name_str));
        }
    }
}
    ...
    match c {
        ...
        ConstraintRentExempt::Enforce => quote! {
            if !__anchor_rent.is_exempt(#info.lamports(), #info.try_data_len()?) {
                return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintRentExempt).with_account_name(#name_str));
            }
        },
    }
}
    ...
    match &c.kind {
        InitKind::Token { owner, mint } => {
            let create_account = generate_create_account(
                field,
                quote! {huione_anchor_hpl::token::TokenAccount::LEN},
            ...
            quote! {
                ...
                let #field: #ty_decl = {
                    if !#if_needed || AsRef::<AccountInfo>::as_ref(&#field).owner == &huione_anchor_lang::huione_program::system_program::ID {
                        ...
                        let accounts = huione_anchor_hpl::token::InitializeAccount {
                            account: #field.to_account_info(),
                            mint: #mint.to_account_info(),
                            authority: #owner.to_account_info(),
                            rent: rent.to_account_info(),
                        };
                        let cpi_ctx = huione_anchor_lang::context::CpiContext::new(cpi_program, accounts);
                        huione_anchor_hpl::token::initialize_account(cpi_ctx)?;
                    }

                    let pa: #ty_decl = #from_account_info_unchecked;
                    if #if_needed {
                        if pa.mint != #mint.key() {
                            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintTokenMint).with_account_name(#name_str).with_pubkeys((pa.mint, #mint.key())));
                        }
                        if pa.owner != #owner.key() {
                            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintTokenOwner).with_account_name(#name_str).with_pubkeys((pa.owner, #owner.key())));
                        }
                    }
                    pa
                };
            ...
            quote! {
                ...
                let #field: #ty_decl = {
                    if !#if_needed || AsRef::<AccountInfo>::as_ref(&#field).owner == &huione_anchor_lang::huione_program::system_program::ID {
                        ...
                        let cpi_accounts = huione_anchor_hpl::associated_token::Create {
                        ...
                        let cpi_ctx = huione_anchor_lang::context::CpiContext::new(cpi_program, cpi_accounts);
                        huione_anchor_hpl::associated_token::create(cpi_ctx)?;
                    }
                    let pa: #ty_decl = #from_account_info_unchecked;
                    if #if_needed {
                        if pa.mint != #mint.key() {
                        ...
                        if pa.key() != huione_anchor_hpl::associated_token::get_associated_token_address(&#owner.key(), &#mint.key()) {
                            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::AccountNotAssociatedTokenAccount).with_account_name(#name_str));
                        }
                    }
                    pa
                };
        ...
        InitKind::Mint {
            ...
            let create_account = generate_create_account(
                field,
                quote! {huione_anchor_hpl::token::Mint::LEN},
                quote! {&token_program.key()},
                seeds_with_bump,
            );
            let freeze_authority = match freeze_authority {
                Some(fa) => quote! { Option::<&huione_anchor_lang::prelude::Pubkey>::Some(&#fa.key()) },
                None => quote! { Option::<&huione_anchor_lang::prelude::Pubkey>::None },
            };
            ...
            quote! {
                ...
                let #field: #ty_decl = {
                    if !#if_needed || AsRef::<AccountInfo>::as_ref(&#field).owner == &huione_anchor_lang::huione_program::system_program::ID {
                        ...
                        let cpi_program = token_program.to_account_info();
                        let accounts = huione_anchor_hpl::token::InitializeMint {
                            mint: #field.to_account_info(),
                            rent: rent.to_account_info(),
                        };
                        let cpi_ctx = huione_anchor_lang::context::CpiContext::new(cpi_program, accounts);
                        huione_anchor_hpl::token::initialize_mint(cpi_ctx, #decimals, &#owner.key(), #freeze_authority)?;
                    }
                    let pa: #ty_decl = #from_account_info_unchecked;
                    if #if_needed {
                        if pa.mint_authority != huione_anchor_lang::huione_program::program_option::COption::Some(#owner.key()) {
                            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintMintMintAuthority).with_account_name(#name_str));
                        }
                        if pa.freeze_authority
                            ...
                            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintMintFreezeAuthority).with_account_name(#name_str));
                        }
                        if pa.decimals != #decimals {
                            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintMintDecimals).with_account_name(#name_str).with_values((pa.decimals, #decimals)));
                        }
                    }
                    pa
                };
        ...
        InitKind::MintMeta {
            ...
            // let create_mint_meta_account = generate_create_mint_meta_account(
            //     field,
            //     quote! {huione_anchor_hpl::token::MintMeta::LEN},
            //     quote! {&token_program.key()},
            //     seeds_with_bump,
            // );
            quote! {
                ...
                let #field: #ty_decl = {
                    if !#if_needed || AsRef::<AccountInfo>::as_ref(&#field).owner == &huione_anchor_lang::huione_program::system_program::ID {
                        ...
                        // Create the account with the system program.
                        // #create_mint_meta_account


                        let cpi_ct_program = token_program.to_account_info();
                        let accounts = huione_anchor_hpl::token::CreateMintMetaAccount {
                            token_program: token_program.to_account_info(),
                            payer:payer.to_account_info(),
                            mint: #mint.to_account_info(),
                            mint_meta: #field.to_account_info(),
                            system_program: system_program.to_account_info(),
                            rent: rent.to_account_info(),

                        };
                        let cpi_ct_ctx = huione_anchor_lang::context::CpiContext::new(cpi_ct_program, accounts);
                        huione_anchor_hpl::token::create_mint_meta_account(cpi_ct_ctx)?;



                        // Initialize the mint account.
                        msg!("generate_create_account over!");
                        let cpi_program = token_program.to_account_info();
                        let accounts = huione_anchor_hpl::token::InitializeMintMeta {
                        ...
                        let cpi_ctx = huione_anchor_lang::context::CpiContext::new(cpi_program, accounts);
                        huione_anchor_hpl::token::initialize_mint_meta(cpi_ctx, symbol,name,icon)?;
                        msg!("initialize_mint_meta over!");
                    }
                    let pa: #ty_decl = #from_account_info_unchecked;
                    if #if_needed {
                        // let test = (#symbol).clone();
                        // if pa.symbol != test {
                        //     // return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintMintDecimals).with_account_name(#name_str.clone()).with_values((pa.symbol.clone(), #symbol.clone())));
                        // }
                        // if pa.name != #name {
                        //     return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintMintDecimals).with_account_name(#name_str.clone()).with_values((pa.name.clone(), #name.clone())));
                        // }
                        // if pa.icon != #icon {
                        //     return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintMintDecimals).with_account_name(#name_str.clone()).with_values((pa.icon.clone(), #icon.clone())));
                        // }
                    }
                    pa
                };
        ...
        InitKind::Program { owner } => {
            ...
            quote! {
                ...
                let #field = {
                    ...
                    let pa: #ty_decl = if !#if_needed || actual_owner == &huione_anchor_lang::huione_program::system_program::ID {
                    ...
                    if #if_needed {
                        if space != actual_field.data_len() {
                            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintSpace).with_account_name(#name_str).with_values((space, actual_field.data_len())));
                        }

                        if actual_owner != #owner {
                            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintOwner).with_account_name(#name_str).with_pubkeys((*actual_owner, *#owner)));
                        }
    ...
    if c.is_init && c.bump.is_some() {
        ...
        quote! {
            if #name.key() != __pda_address {
                return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintSeeds).with_account_name(#name_str).with_pubkeys((#name.key(), __pda_address)));
            }
            if __bump != #b {
                return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintSeeds).with_account_name(#name_str).with_values((__bump, #b)));
            }
        }
    }
        ...
        let define_pda = match c.bump.as_ref() {
            ...
            Some(b) => quote! {
                let __pda_address = Pubkey::create_program_address(
                    ...
                    &#deriving_program_id,
                ).map_err(|_| huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintSeeds).with_account_name(#name_str))?;
            },
        };
        quote! {
            // Define the PDA.
    ...
    quote! {
        {
            ...
            if my_owner != wallet_address {
                return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintTokenOwner).with_account_name(#name_str).with_pubkeys((my_owner, wallet_address)));
            }
            let __associated_token_address = huione_anchor_hpl::associated_token::get_associated_token_address(&wallet_address, &#spl_token_mint_address.key());
            let my_key = #name.key();
            if my_key != __associated_token_address {
                return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintAssociated).with_account_name(#name_str).with_pubkeys((my_key, __associated_token_address)));
            }
        }
    }
}
...
fn generate_constraint_token_account(
    ...
    let authority_check = match &c.authority {
        Some(authority) => {
            quote! { if #name.owner != #authority.key() { return Err(huione_anchor_lang::error::ErrorCode::ConstraintTokenOwner.into()); } }
        }
        None => quote! {},
    };
    let mint_check = match &c.mint {
        Some(mint) => {
            quote! { if #name.mint != #mint.key() { return Err(huione_anchor_lang::error::ErrorCode::ConstraintTokenMint.into()); } }
        }
        None => quote! {},
    };
    quote! {
...
fn generate_constraint_mint(f: &Field, c: &ConstraintTokenMintGroup) -> proc_macro2::TokenStream {
    ...
    let decimal_check = match &c.decimals {
        Some(decimals) => quote! {
            if #name.decimals != #decimals {
                return Err(huione_anchor_lang::error::ErrorCode::ConstraintMintDecimals.into());
            }
        },
        None => quote! {},
    };
    let mint_authority_check = match &c.mint_authority {
        Some(mint_authority) => quote! {
            if #name.mint_authority != huione_anchor_lang::huione_program::program_option::COption::Some(huione_anchor_lang::Key::key(&#mint_authority)) {
                return Err(huione_anchor_lang::error::ErrorCode::ConstraintMintMintAuthority.into());
            }
        },
        None => quote! {},
    };
    let freeze_authority_check = match &c.freeze_authority {
        Some(freeze_authority) => quote! {
            if #name.freeze_authority != huione_anchor_lang::huione_program::program_option::COption::Some(huione_anchor_lang::Key::key(&#freeze_authority)) {
                return Err(huione_anchor_lang::error::ErrorCode::ConstraintMintFreezeAuthority.into());
            }
        },
        None => quote! {},
    };
...
fn generate_constraint_mint_meta(f: &Field, c: &ConstraintTokenMintMetaGroup) -> proc_macro2::TokenStream {
    // let name = &f.ident;
...
pub fn generate_create_account(
    ...
    quote! {
        ...
        if __current_lamports == 0 {
            ...
            let cpi_accounts = huione_anchor_lang::system_program::CreateAccount {
                from: payer.to_account_info(),
                to: #field.to_account_info()
            };
            let cpi_context = huione_anchor_lang::context::CpiContext::new(system_program.to_account_info(), cpi_accounts);
            msg!("generate_create_account");
            huione_anchor_lang::system_program::create_account(cpi_context.with_signer(&[#seeds_with_nonce]), lamports, #space as u64, #owner)?;
        } else {
            ...
            if required_lamports > 0 {
                let cpi_accounts = huione_anchor_lang::system_program::Transfer {
                    from: payer.to_account_info(),
                    to: #field.to_account_info(),
                };
                let cpi_context = huione_anchor_lang::context::CpiContext::new(system_program.to_account_info(), cpi_accounts);
                huione_anchor_lang::system_program::transfer(cpi_context, required_lamports)?;
            }
            // Allocate space.
            let cpi_accounts = huione_anchor_lang::system_program::Allocate {
                account_to_allocate: #field.to_account_info()
            };
            let cpi_context = huione_anchor_lang::context::CpiContext::new(system_program.to_account_info(), cpi_accounts);
            huione_anchor_lang::system_program::allocate(cpi_context.with_signer(&[#seeds_with_nonce]), #space as u64)?;
            // Assign to the spl token program.
            let cpi_accounts = huione_anchor_lang::system_program::Assign {
                account_to_assign: #field.to_account_info()
            };
            let cpi_context = huione_anchor_lang::context::CpiContext::new(system_program.to_account_info(), cpi_accounts);
            huione_anchor_lang::system_program::assign(cpi_context.with_signer(&[#seeds_with_nonce]), #owner)?;
        }
    }
}
...
// `seeds_with_nonce` should be given for creating PDAs. Otherwise it's an
// empty stream.
pub fn generate_create_mint_meta_account(
    field: &Ident,
    space: proc_macro2::TokenStream,
    owner: proc_macro2::TokenStream,
    seeds_with_nonce: proc_macro2::TokenStream,
    ...
    quote! {
        ...
        // let cpi_meta_program = token_program.to_account_info();
        // let accounts = huione_anchor_hpl::token::CreateMintAccount {
        //     payer:payer.to_account_info(),
        //     mint: mint.to_account_info(),
        //     mint_meta: #field.to_account_info(),
        //     token_program: token_program.to_account_info(),
        ...
        // };
        // let cpi_ctx = huione_anchor_lang::context::CpiContext::new(cpi_program, accounts);
        // huione_anchor_hpl::token::create_mint_meta_account(cpi_ctx)?;
...
pub fn generate_constraint_executable(
    ...
    quote! {
        if !#name.to_account_info().executable {
            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintExecutable).with_account_name(#name_str));
        }
    }
}
...
pub fn generate_constraint_state(f: &Field, c: &ConstraintState) -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        // the target program.
        if #ident.key() != huione_anchor_lang::accounts::cpi_state::CpiState::<#account_ty>::address(&#program_target.key()) {
            return Err(huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::ConstraintState).with_account_name(#name_str));
        }
...
fn generate_custom_error(
    ...
    let mut error = match custom_error {
        Some(error) => {
            quote! { huione_anchor_lang::error::Error::from(#error).with_account_name(#account_name) }
        }
        None => {
            quote! { huione_anchor_lang::error::Error::from(huione_anchor_lang::error::ErrorCode::#error).with_account_name(#account_name) }
        }
    };
