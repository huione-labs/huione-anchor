...
pub fn generate(accs: &AccountsStruct) -> proc_macro2::TokenStream {
    ...
    let account_struct_fields: Vec<proc_macro2::TokenStream> = accs
        ...
        .map(|f: &AccountField| match f {
            ...
            AccountField::Field(f) => {
                ...
                quote! {
                    #docs
                    pub #name: huione_anchor_lang::huione_program::account_info::AccountInfo<'info>
                }
            }
        })
        .collect();
        ...
        .map(|f: &AccountField| match f {
            ...
            AccountField::Field(f) => {
                ...
                let meta = match f.constraints.is_mutable() {
                    false => quote! { huione_anchor_lang::huione_program::instruction::AccountMeta::new_readonly },
                    true => quote! { huione_anchor_lang::huione_program::instruction::AccountMeta::new },
                };
                let name = &f.ident;
                quote! {
                    account_metas.push(#meta(huione_anchor_lang::Key::key(&self.#name), #is_signer));
                }
            }
        })
        .collect();
    ...
    let account_struct_infos: Vec<proc_macro2::TokenStream> = accs
        ...
        .map(|f: &AccountField| match f {
            AccountField::CompositeField(s) => {
                let name = &s.ident;
                quote! {
                    account_infos.extend(huione_anchor_lang::ToAccountInfos::to_account_infos(&self.#name));
                }
            }
            AccountField::Field(f) => {
                let name = &f.ident;
                quote! {
                    account_infos.push(huione_anchor_lang::ToAccountInfo::to_account_info(&self.#name));
                }
            }
        })
        .collect();
    ...
    quote! {
        ...
        pub(crate) mod #account_mod_name {
            ...
            impl #generics huione_anchor_lang::ToAccountMetas for #name #generics {
                fn to_account_metas(&self, is_signer: Option<bool>) -> Vec<huione_anchor_lang::huione_program::instruction::AccountMeta> {
            ...
            #[automatically_derived]
            impl<'info> huione_anchor_lang::ToAccountInfos<'info> for #name #generics {
                fn to_account_infos(&self) -> Vec<huione_anchor_lang::huione_program::account_info::AccountInfo<'info>> {
