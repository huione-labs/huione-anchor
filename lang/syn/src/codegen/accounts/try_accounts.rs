...
pub fn generate(accs: &AccountsStruct) -> proc_macro2::TokenStream {
            ...
            match af {
                AccountField::CompositeField(s) => {
                    ...
                    quote! {
                        ...
                        ::huione_program::log::sol_log(stringify!(#name));
                        let #name: #ty = huione_anchor_lang::Accounts::try_accounts(program_id, accounts, ix_data, __bumps)?;
                    }
                }
                AccountField::Field(f) => {
                    ...
                    if is_init(af) || f.constraints.zeroed.is_some() {
                        ...
                        quote! {
                            #[cfg(feature = "anchor-debug")]
                            ::huione_program::log::sol_log(stringify!(#typed_name));
                            let #typed_name = huione_anchor_lang::Accounts::try_accounts(program_id, accounts, ix_data, __bumps)
    ...
    quote! {
        ...
        impl<#combined_generics> huione_anchor_lang::Accounts<#trait_generics> for #name<#struct_generics> #where_clause {
            #[inline(never)]
            fn try_accounts(
                program_id: &huione_anchor_lang::huione_program::pubkey::Pubkey,
                accounts: &mut &[huione_anchor_lang::huione_program::account_info::AccountInfo<'info>],
                ix_data: &[u8],
                __bumps: &mut std::collections::BTreeMap<String, u8>,
            ) -> huione_anchor_lang::Result<Self> {
