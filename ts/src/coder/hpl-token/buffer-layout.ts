...
export function uint128(property?: string): Layout<u128 | null> {
  return new WrappedLayout(
    BufferLayout.blob(16),
    (b: Buffer) => u128.fromBuffer(b),
    (n: u128) => n.toBuffer(),
    property
  );
}
...
export class u128 extends BN {
  ...
  toBuffer(): Buffer {
    ...
    if (b.length >= 16) {
      throw new Error("u128 too large");
    }
  ...
  static fromBuffer(buffer: Buffer): u128 {
    if (buffer.length !== 16) {
      throw new Error(`Invalid u128 buffer length: ${buffer.length}`);
    }
    return new u128(
      [...buffer]
        .reverse()
